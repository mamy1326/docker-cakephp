                ...
                <div class="box-body table-responsive no-padding">
                    ...
                    <table class="table table-hover text-nowrap dataTable table-striped table-bordered table-condensed" style="font-size:13px;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?= h($authority->id) ?></td>
                                <td><?= h($authority->name) ?></td>
                                <td><?= h($authority->description) ?></td>
                                <td><?= h($authority->created) ?><br><?= h($authority->modified) ?></td>
                                <td><?= $authority->delete_icon ?><?= h($authority->deleted) ?></td>
                                <td class="actions">
                                    <?= $this->Html->link(__('編集'), ['action' => 'edit', $authority->id], ['class' => 'btn btn-primary btn-xs']) ?>
                                    <?=
                                        $this->Form->postLink(
                                            __('削除'),
                                            [
